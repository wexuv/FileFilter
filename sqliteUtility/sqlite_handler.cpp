...
namespace driver
{
	...
	int SQLite_Handler::open (const char* file_name)
	{
		if (is_opened_)
		{
			this->close ();
	...
	int SQLite_Handler::execute (const char* sql)
	{
		if (NULL == sql)
		{
			return fail;
	...
	int SQLite_Handler::execute (const char* sql, SQLite_Result& dbresult)
	{
		if (NULL == sql)
		{
			return fail;
